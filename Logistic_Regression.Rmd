...
# Normalizing our Data with Standardization
```{r, echo=FALSE, message=FALSE, warning=FALSE}
preproc1 <- preProcess(WW_LR_data, method = c("center", "scale"))

w_dat_stand <- predict(preproc1, WW_LR_data) 
...
w_dat_minmax <- predict(preproc2, WW_LR_data)

summary(w_dat_minmax)
```
...
LR.fit2 = glm(like ~ volatile.acidity + residual.sugar + free.sulfur.dioxide + 
                density + pH + sulphates + alcohol, data = w_dat_stand, family = binomial)
#summary(LR.fit2)
#plot(LR.fit2)
LR.probs2 = predict(LR.fit2, w_test, type = "response")
LR.pred2 = rep(0, 4898)
LR.pred2[LR.probs2>0.5] = 1
table(LR.pred2, w_dat_stand$like)
mean(LR.pred2==w_dat_stand$like)
...
LR.BIC7 = glm(like ~ volatile.acidity + residual.sugar + free.sulfur.dioxide + 
                density + pH + sulphates + alcohol , data = w_dat_minmax, family =
                binomial)
#summary(LR.fit4)
#plot(LR.fit4)
...
table(LR.pred4, w_dat_minmax$like)
mean(LR.pred4==w_dat_minmax$like)
```
