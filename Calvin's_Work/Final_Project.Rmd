...
levels_of_significance = c("NSF", "(- -)", "NSF", "NSF", "(- -)", "NSF", "NSF", "(- - -)", "NSF", "NSF", "(+++)")
...
# Scatterplots to Demonstrate the Correlations
```{r, echo=FALSE, message=FALSE, warning=FALSE}
#Showing alcohol levels against quality
alcohol_summary <- twentypercent %>% group_by(quality) %>% summarise(alcohol = mean(alcohol))

alcohol_scp <- ggplot(twentypercent, aes(x = quality, y = alcohol)) + geom_point(alpha = 1/10, position = position_jitter(height = 0.2, width = 0.2), color = "blue3") + geom_line(data = alcohol_summary, size = 2, color = "yellow") + geom_point(data = alcohol_summary, size = 4, color = "yellow")

#Showing density levels against quality
density_summary <- twentypercent %>% group_by(quality) %>% summarise(density = mean(density))

density_scp <- ggplot(twentypercent, aes(x = quality, y = density)) + geom_point(alpha = 1/10, position = position_jitter(height = 0.2, width = 0.2), color = "blue3") + geom_line(data = density_summary, size = 2, color = "yellow") + geom_point(data = density_summary, size = 4, color = "yellow")
...
volatile_acidity_scp <- ggplot(twentypercent, aes(x = quality, y = volatile.acidity)) + geom_point(alpha = 1/10, position = position_jitter(height = 0.2, width = 0.2), color = "blue3") + geom_line(data = volatile_acidity_summary) + geom_point(data = volatile_acidity_summary, size = 4, color = "yellow")

#Showing chlorides against quality
chloride_summary <- twentypercent %>% group_by(quality) %>% summarise(chlorides = mean(chlorides))

chlorides_scp <- ggplot(twentypercent, aes(x = quality, y = chlorides)) + geom_point(alpha = 1/10, position = position_jitter(height = 0.2, width = 0.2), color = "blue3") + geom_line(data = chloride_summary, size = 2, color = "yellow") + geom_point(data = chloride_summary, size = 4, color = "yellow")

plot_grid(alcohol_scp, density_scp, volatile_acidity_scp, chlorides_scp, ncol = 2, nrow = 2)
```
